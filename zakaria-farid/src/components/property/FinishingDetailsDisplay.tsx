...
function getAttrLabel(attrTemplateId: string, locale: string, customLabel?: string): string {
        ...
        <div className={styles.revampedAttrGrid}>
          {filledAttrs.map(attr => (
            <div key={attr.attribute_template_id} className={styles.revampedAttrChip}>
              <span className={styles.revampedAttrKey}>{getAttrLabel(attr.attribute_template_id, locale, attr.custom_label)}:</span>
                    ...
                    <div className={styles.reportAttrGrid}>
                      {filledAttrs.map(attr => (
                        <div key={attr.attribute_template_id} className={styles.reportAttrRow}>
                          <span className={styles.reportAttrKey}>{getAttrLabel(attr.attribute_template_id, locale, attr.custom_label)}:</span>
      ...
      <div className={styles.reportModalDialog} onClick={e => e.stopPropagation()} dir={isAr ? 'rtl' : 'ltr'}>
        ...
        <div className={styles.reportTopBar}>
          <div className={styles.reportBrandTitle}>
            ...
            <span>{isAr ? 'آل زكريا — تقرير المواصفات والتشطيب' : 'AL ZAKARIA — Official Finishing Spec Sheet'}</span>
          </div>
        ...
        <div className={styles.reportDocumentBody} id="printable-spec-sheet">
          ...
          <div className={styles.reportDocFooter}>
            <p>{isAr ? 'تقرير رسمي صادر عن منصة آل زكريا للعقارات الفاخرة والاستشارات السيادية' : 'Official finishing specification generated by AL ZAKARIA Sovereign Real Estate Platform'}</p>
          </div>
        </div>
