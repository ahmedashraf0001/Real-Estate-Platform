...
import { createLead, updateLeadDetails, updateLeadStage } from '@/app/actions/leads';
  ...
  const grouped = useMemo(
    () =>
      STAGE_CONFIG.map((stage) => ({
        ...stage,
        items: leads.filter((lead) => (lead.stage || 'new') === stage.key),
      })),
    [leads]
  );
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <span style={{
              ...
              {leads.length} {isAr ? 'طلب إجمالي' : 'Active Pipeline Leads'}
            </span>
          </div>
          <p style={{ margin: '3px 0 0', color: '#64748B', fontSize: '13px' }}>
            {isAr
              ? 'قم بسحب وإسقاط البطاقات لتغيير مراحل المبيعات بسهولة'
              : 'Drag & drop lead cards between stage columns to advance your sales pipeline. Drag to left/right edge to scroll.'}
          </p>
        </div>
                  ...
                  {stage.items.map((lead) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Card Footer: Time ago & Functional View Property Link */}
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '10px', color: '#64748B', paddingTop: '4px', borderTop: '1px dashed #E2E8F0' }}>
                          ...
                          {lead.property ? (
                            ...
                            <span style={{ fontSize: '10px', color: '#94A3B8', fontWeight: 500 }}>
                              {lead.entry_method === 'manual' ? (isAr ? 'إدخال يدوي' : 'Manual Entry') : (isAr ? 'استفسار عام' : 'General Inquiry')}
                            </span>
                          )}
                        </div>
                      </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', flex: 1 }}>
              ...
              <div style={{ marginTop: 'auto', paddingTop: '10px' }}>
                ...
                >
                  <Save size={14} />
                  <span>{isAr ? 'حفظ التغييرات' : 'Save Details'}</span>
                </button>
              </div>
            </div>
          </div>
